...
void PrintFullEntry(LSTR* FullBaseDir, WIN32_FIND_DATA* finddata, LineWriter* outputLine, bool printOwner, LPCWSTR owner, bool printQuoted)
{
	...
	outputLine->appendf(L"\\%s%s\r\n", finddata->cFileName, printQuoted ? L"\"" : L"");

	outputLine->writeBuffer();
}

void PrintEntry(LSTR *FullBaseDir, WIN32_FIND_DATA *finddata, LineWriter *outputLine, bool printFull, bool printOwner, bool printQuoted)
{
	WCHAR owner[128];
	...
	if (printFull)
	{
		PrintFullEntry(FullBaseDir, finddata, outputLine, printOwner, owner, printQuoted);
	}
	else
	{
